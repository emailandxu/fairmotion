...
import math
import numpy as np
from fairmotion.core.motion import ComparablePose
from fairmotion.core.velocity import ComparableVelocity
...
def my_pose_similarity(
    ...
    if w_joints is None:
        w_joints = np.ones(pose1.skel.num_joints())

    pose1 = ComparablePose.fromPose(pose1)
    pose2 = ComparablePose.fromPose(pose2)

    if vel1 is not None and vel2 is not None:
        vel1 = ComparableVelocity.fromVelocity(vel1)
        vel2 = ComparableVelocity.fromVelocity(vel2)


    if w_joint_pos > 0.0:
        diff_pos = pose1.diff(pose2)
        diff_pos = np.dot(w_joints, diff_pos)
    else:
        diff_pos = 0.0

    if vel1 is not None and w_joint_vel > 0.0:
        diff_vel = vel1.diff(vel2)
        diff_vel = np.dot(w_joints, diff_vel)
    else:
        diff_vel = 0.0
    ...
    diff_pos /= pose1.skel.num_joints()
    diff_vel /= pose1.skel.num_joints()

    edge_weight = w_joint_pos * diff_pos + w_joint_vel * diff_vel

    return np.array( [edge_weight, diff_pos, diff_vel] )
    

def my_root_ee_similarity(
    pose1,
    pose2,
    vel1=None,
    vel2=None,
    ...
    return np.array( [diff, diff_root_pos, diff_root_vel, diff_ee_pos, diff_ee_vel] )